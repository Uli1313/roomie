import{a as d,S as r}from"./bootstrap.min-b3783de9.js";const _="https://roomie-lfta.onrender.com",F=localStorage.getItem("userId"),U=localStorage.getItem("token"),Y={headers:{Authorization:`Bearer ${U}`}};let C;const v=document.querySelectorAll(".nav-logged"),$=document.querySelectorAll(".nav-unlogged"),P=document.querySelectorAll(".nav-logged-photo"),E=document.querySelector(".logout-btn");(async()=>{try{const s=`/600/users/${F}`,e=`${_}${s}`,t=await d.get(e,Y),a=t.data.photo;P.forEach(o=>o.setAttribute("src",a)),C=t.data,H(),v.forEach(o=>{o.classList.remove("d-none")}),$.forEach(o=>{o.classList.add("d-none")})}catch(s){console.log(s),v.forEach(e=>{e.classList.add("d-none")}),$.forEach(e=>{e.classList.remove("d-none")})}})();function H(){localStorage.setItem("user",JSON.stringify(C))}E.addEventListener("click",z);function z(){r.fire({icon:"question",title:"確定要登出嗎",showCancelButton:!0,confirmButtonText:"確定",cancelButtonText:"取消"}).then(s=>{s.isConfirmed&&(localStorage.removeItem("userId"),localStorage.removeItem("token"),localStorage.removeItem("user"),r.fire({icon:"success",title:"已登出",timer:1500,showConfirmButton:!1}),setTimeout(()=>{location.href="login.html"},1e3))})}const p="https://roomie-lfta.onrender.com",m=localStorage.getItem("userId"),R=localStorage.getItem("token"),c={headers:{Authorization:`Bearer ${R}`}};let w,x,L,k;const B=document.querySelector("#publishList"),T=document.querySelector("#matchedList"),S=document.querySelector("#removedList"),D=document.querySelector("#draftList"),J=document.querySelectorAll(".user-post-list"),N=document.querySelector("#postsTab");let j="publish-tab";function y(){const s=`${p}/600/rents?userId=${m}&status=刊登中`;d.get(s,c).then(e=>{w=e.data,O(w)}).catch(e=>console.log(e))}function O(s){if(!s.length){B.innerHTML=`<div class="d-flex justify-content-center align-items-center">
    <p>尚無相關物件 ¯_(ツ)_/¯</p>
 ...
 ${a}
  ...
  </div>`},"")}function q(){const s=`${p}/600/rents?userId=${m}&status=已媒合`;d.get(s,c).then(e=>{x=e.data,V(x)}).catch(e=>console.log(e))}function V(s){let e="";if(!s.length){T.innerHTML=`<div class="d-flex justify-content-center align-items-center">
    <p>尚無相關物件 ¯_(ツ)_/¯</p>
  </div>`;return}s.forEach(function(t){const a=`${t.updateDate}`,o=new Date(a),i=`${t.soldDate}`,n=new Date(i),l=new Date,f=o.getTime()-n.getTime(),b=n.getTime()-l.getTime(),h=Math.abs(Math.trunc(f/(1e3*3600*24))),M=Math.abs(Math.trunc(b/(1e3*3600*24)));e+=`<div class="user-post-item col-12 my-3 p-1 hover-primary-2 rounded">
                ...
                `}),T.innerHTML=e}function I(){const s=`${p}/600/rents?userId=${m}&status=下架`;d.get(s,c).then(e=>{L=e.data,W(L)}).catch(e=>console.log(e))}function W(s){if(!s.length){S.innerHTML=`<div class="d-flex justify-content-center align-items-center">
    <p>尚無相關物件 ¯_(ツ)_/¯</p>
 ...
 ${a}
  ...
  </div>`},"")}function A(){const s=`${p}/600/rents?userId=${m}&status=草稿`;d.get(s,c).then(e=>{k=e.data,G(k)}).catch(e=>console.log(e))}function G(s){if(!s.length){D.innerHTML=`<div class="d-flex justify-content-center align-items-center">
    <p>尚無相關物件 ¯_(ツ)_/¯</p>
 ...
 ${a}
  ...
  </div>`},"")}N.addEventListener("click",s=>{const e=s.target.getAttribute("id");switch(j=e,e){case"publish-tab":y();break;case"matched-tab":q();break;case"removed-tab":I();break;case"draft-tab":A();break}});J.forEach(s=>{s.addEventListener("click",e=>{const t=e.target.classList;if(t.contains("to-removed"))return g("下架",e);if(t.contains("to-matched"))return g("已媒合",e);if(t.contains("to-publish"))return g("刊登中",e);if(t.contains("delete-post"))return K(e)})});function g(s,e){const t=e.target.closest(".user-post-item").getAttribute("data-post-id"),a=`${p}/600/rents/${t}`,o={status:s};let i;if(s==="已媒合"){const n=new Date,l=n.getFullYear(),f=(n.getMonth()+1).toString().padStart(2,"0"),b=n.getDate().toString().padStart(2,"0"),h=`${l}/${f}/${b}`;i={status:s,soldDate:h}}switch(s){case"下架":r.fire({icon:"question",title:"確定要暫時下架嗎",text:"點選確定後將移至下架區，可以至下架區再次上架貼文",showCancelButton:!0,confirmButtonText:"確定",cancelButtonText:"取消"}).then(n=>{n.isConfirmed&&d.patch(a,o,c).then(l=>{u(),r.fire({icon:"success",title:"下架貼文成功",timer:1500,showConfirmButton:!1})}).catch(l=>{r.fire({icon:"error",title:"下架貼文失敗",timer:1500,showConfirmButton:!1}),console.log(l)})});break;case"已媒合":r.fire({icon:"question",title:"確定媒合成功嗎",text:"一旦點選確定後將移至已媒合，無法再次上架貼文",input:"checkbox",inputPlaceholder:"我同意將貼文保留在媒合成功頁面",inputValidator:n=>!n&&"請勾選同意",showCancelButton:!0,confirmButtonText:"確定",cancelButtonText:"取消"}).then(n=>{n.isConfirmed&&d.patch(a,i,c).then(l=>{u(),r.fire({icon:"success",title:"恭喜您媒合成功",timer:1500,showConfirmButton:!1})}).catch(l=>{r.fire({icon:"error",title:"發生錯誤",timer:1500,showConfirmButton:!1})})});break;case"刊登中":r.fire({icon:"question",title:"確定要上架貼文嗎",showCancelButton:!0,confirmButtonText:"確定",cancelButtonText:"取消"}).then(n=>{n.isConfirmed&&d.patch(a,o,c).then(l=>{u(),r.fire({icon:"success",title:"上架貼文成功",timer:1500,showConfirmButton:!1})}).catch(l=>{r.fire({icon:"error",title:"上架貼文失敗",timer:1500,showConfirmButton:!1})})});break}}function K(s){const e=s.target.closest(".user-post-item").getAttribute("data-post-id"),t=`${p}/600/rents/${e}`;r.fire({icon:"question",title:"確定要刪除貼文嗎",text:"一旦刪除後將無法復原",showCancelButton:!0,confirmButtonText:"確定",cancelButtonText:"取消"}).then(a=>{a.isConfirmed&&d.delete(t,c).then(o=>{u(),r.fire({icon:"success",title:"刪除貼文成功",timer:1500,showConfirmButton:!1})}).catch(o=>{r.fire({icon:"error",title:"刪除貼文失敗",timer:1500,showConfirmButton:!1})})})}function u(){switch(j){case"publish-tab":y();break;case"matched-tab":q();break;case"removed-tab":I();break;case"draft-tab":A();break}}function Q(){y()}Q();[].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'));
